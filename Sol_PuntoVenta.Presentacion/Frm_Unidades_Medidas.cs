...
namespace Sol_PuntoVenta.Presentacion
{
    public partial class Frm_Unidades_Medidas : Form
    {
        ...
        private void Formato_um()
        {
            ...
            Dgv_Listado.Columns[1].HeaderText="UNIDADES DE MEDIDA";
        }
        ...
        private void Listado_um(string cTexto)
        {
            try
            {
                Dgv_Listado.DataSource = N_Unidades_Medida.Listado_um(cTexto);
                this.Formato_um();
        ...
        private void Selecciona_Item()
        {
            ...
            else
            {
                ...
                txtDescripcion.Text = Convert.ToString((Dgv_Listado.CurrentRow.Cells["descripcion_um"].Value));
            }
        }
        #endregion
        ...
        private void Btn_guardar_Click(object sender, EventArgs e)
        {
            try
            {
                if (txtDescripcion.Text == String.Empty)
                {
                    ...
                    oPropiedad.Descripcion_um = txtDescripcion.Text.Trim();
                    rpta = N_Unidades_Medida.Guardar_um(this.Estadoguarda, oPropiedad);
                   if (rpta.Equals("OK"))
                    {
        ...
        private void Btn_eliminar_Click(object sender, EventArgs e)
        {
            if (Dgv_Listado.Rows.Count > 0)
            {
                ...
                if (option == DialogResult.Yes)
                {
                    string rpta = "";
                    this.Selecciona_Item();
                    rpta = N_Unidades_Medida.Eliminar_um(this.nCodigo);
                    if (rpta.Equals("OK"))
                    {
                        this.Listado_um("%");
        ...
        private void Btn_reporte_Click(object sender, EventArgs e)
        {
            if (Dgv_Listado.Rows.Count>0)
            {
                    Reportes.Frm_Rpt_Unidad_Medida oRpt_ma = new Reportes.Frm_Rpt_Unidad_Medida();
                    oRpt_ma.txt_p1.Text = txtBuscar.Text.Trim();
                    oRpt_ma.ShowDialog(); 
            }
        }
